...
class Replay(object):
	...
	# def __init__(self, window_width, window_height, num_hiders, num_seekers, polygon_map, conf_options):
		...
		self.__window = graphics.Graphics(window_width, window_height, self.__num_hiders, self.__num_seekers, self.__map, self.__conf_options)
	...
	def run_replay(self):
		pyglet.clock.schedule_interval(self.update, 1/self.__fps) # update at 60Hz
		pyglet.app.run()
